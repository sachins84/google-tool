...
export async function videoAssetRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/', async (req, reply) => {
    ...
    async function fetchWindow(from: string, to: string): Promise<VideoRow[]> {
      ...
      const perAccount = await Promise.all(
        brand!.accounts.map(async (acc) => {
          const aggregated = new Map<string, { row: VideoRow; raw: RawMetrics; usageMap: Map<string, Usage> }>();
          try {
            ...
            for (const r of pmaxRows) {
              ...
              addUsage(aggregated, ytId, r.asset?.youtubeVideoAsset?.youtubeVideoTitle, r.asset?.id, raw, {
          ...
          return Array.from(aggregated.values()).map(({ row, raw, usageMap }) => ({
            ...row,
      ...
      const merged = new Map<string, { row: VideoRow; raw: RawMetrics }>();
      for (const acctRows of perAccount) {
        for (const v of acctRows) {
          ...
          if (!existing) {
            ...
            merged.set(v.youtube_video_id, { row: v, raw: rawSum });
          } else {
      ...
      return Array.from(merged.values()).map(({ row, raw }) => ({
        ...row,
        metrics: applyFlatRto(deriveMetrics(raw), brand!.rto_factor),
      }));
    }
...
function addUsage(
  aggregated: Map<string, { row: VideoRow; raw: RawMetrics; usageMap: Map<string, Usage> }>,
  ytId: string,
